...
def writeTSPLIBfile_FE(fname_tsp, CostMatrix, user_comment):
    ...
    display_data_type_line = 'DISPLAY_DATA_TYPE: ' + \
        'NO_DISPLAY' + '\n'  # 'NO_DISPLAY'
    ...
    problem_file_line = 'PROBLEM_FILE = ' + pwd + tsplib_dir + \
...
def copy_toTSPLIBdir_cmd(fname_basis):
    copy_toTSPLIBdir_cmd = 'cp' + ' ' + pwd + '/' + \
        fname_basis + '.txt' + ' ' + pwd + tsplib_dir
    os.system(copy_toTSPLIBdir_cmd)


def run_LKHsolver_cmd(fname_basis):
    run_lkh_cmd = pwd + lkh_dir + lkh_cmd + ' ' + \
        pwd + tsplib_dir + fname_basis + '.par'
...
def return_tsp_path_list(fname_basis):
    fname = pwd + tsplib_dir + fname_basis + '.txt'
    path = []
...
def run_tsp(cost_matrix, fname_tsp, user_comment):
    """Run Traveling Salesman solver on initialized viewpoints"""
    [fileID1,fileID2] = writeTSPLIBfile_FE(fname_tsp, cost_matrix, user_comment)
    run_LKHsolver_cmd(fname_tsp)
    copy_toTSPLIBdir_cmd(fname_tsp)
    return return_tsp_path_list(fname_tsp)


def main(): 
    import pickle
    with open(os.path.join(pwd, "CostMatrix.pickle"), 'rb') as f:
        cost_matrix = pickle.load(f)

    cost_matrix = cost_matrix * 100  # add two significant figures to value goes from [m] to [cm]
